...
impl State {
    pub fn encrypt(&self, value: &str) -> Result<String> {
        encrypt(&self.encryption_key, value)
    }
    pub fn decrypt(&self, value: &str) -> Result<String> {
        decrypt(&self.encryption_key, value)
    }
}
pub(crate) fn encrypt(encryption_key: &[u8; 32], value: &str) -> Result<String> {
...
pub(crate) fn decrypt(encryption_key: &[u8; 32], value: &str) -> Result<String> {
    let (nonce, data) = value
        .split_once(':')
        .ok_or_else(|| Error::unavailable("Encrypted secret is invalid"))?;
    let nonce = hex::decode(nonce).map_err(|_| Error::unavailable("Encrypted nonce is invalid"))?;
    if nonce.len() != 12 {
        return Err(Error::unavailable("Encrypted nonce is invalid"));
    }
    let data = hex::decode(data).map_err(|_| Error::unavailable("Encrypted data is invalid"))?;
    let cipher = Aes256Gcm::new_from_slice(encryption_key)
        .map_err(|_| Error::unavailable("Encryption key is invalid"))?;
    let plain = cipher
