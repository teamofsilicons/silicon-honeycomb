## Current contract
The CLI and Rust client are **0.3.0**. General APIs use **HTTP v1**; release-channel APIs use **HTTP v2**. Website release screens use v2 while other screens stay on v1. Package manifests remain **format_version: 1**. Each application maintains independent prod and dev release histories.
