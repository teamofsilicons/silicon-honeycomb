...
`installer::install_archive_for(app_id, archive, state_dir, aliases, previous)` installs an archive under the selected application ID. It validates and stages the payload, checks command collisions, and returns an `Installed` record. Your application owns the installation registry and concurrency lock. Pass the previous record for an update; do not overwrite another package's command. A command of the same name elsewhere on PATH returns `installer::UnresolvedCommands` when it is older than the package needs, carrying the version found and the version required so you can ask before acting; `installer::install_archive_resolving(.., rewrite)` performs the answered rewrite and records what it displaced. `installer::uninstall(record, state_dir)` removes only owned installation artifacts. The existing `install_archive` helper still works when the manifest declares `app_id` or a previous installation supplies it.
