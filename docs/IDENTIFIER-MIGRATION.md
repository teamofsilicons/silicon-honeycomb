...
Historical maintenance attribution is retained verbatim, without creating IAM principals: provider review guidance in discussion rows; accepted `iam.adopt` and `iam.catalog.restore` operation labels; and the exact observed placeholder materialization, catalog deletion, and test reset audit labels. The mapper allows these only for their recorded table, action, production plane, and accepted operation state. A reused label on another action fails closed. These values are historical system annotations, not credentials or canonical Carbon/Silicon identities.

Unregistered drafts rename their requested `local_app_id` field to `app_id` only after validating nonempty handles and matching the draft's explicit owning organization. Empty unfinished handles stay empty, and existing edit-target references use the IAM map. This does not create an IAM application or assign ownership; registration still performs the normal global collision and authorization checks. References to existing applications continue to require the authoritative IAM map.

Terminal publication records (`published`, `denied`, `rejected`, `cancelled`, `withdrawn`) retain their exact review provider namespace, scopes, snapshots, and receipt JSON. A historical coordinator role `honeycomb` and application provider `tos>honeycomb` can coexist; they must never be merged by the migration. Active reviews still block cutover unless the verified hold preconditions are satisfied.
